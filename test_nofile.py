...
class Board:
    ...
    def place_objs(self):
        ...
        for coord in self.gtile_coords:
            self.layout[coord[0]][coord[1]] = G_Tile(coord[1], None)
    ...
    def events(self):
        ...
        for row in self.layout:
            for tile in row:
                ...
                # If all goal tiles contain the box object, then the game is won
                if type(tile) is G_Tile and type(tile.get_contents()) is not Box:
                    print(f'Goal at {tile.get_id()} does not have a box')
                    self.win = False
        if self.win:
            return True
